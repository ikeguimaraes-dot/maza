-- Migration 073_maza_learning_proposals.sql
...
-- ── 1. Tabela principal ──────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS public.maza_learning_proposals (
...
-- ── 2. Índices ────────────────────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_maza_learning_proposals_modulo_status
  ON public.maza_learning_proposals (modulo, status);

CREATE INDEX IF NOT EXISTS idx_maza_learning_proposals_created_at
  ON public.maza_learning_proposals (created_at DESC);

-- ── 3. RLS ───────────────────────────────────────────────────────────────
ALTER TABLE public.maza_learning_proposals ENABLE ROW LEVEL SECURITY;

-- Leitura: qualquer usuário autenticado pode ver propostas
CREATE POLICY "maza_learning_proposals_select"
  ON public.maza_learning_proposals
  FOR SELECT
  TO authenticated
  USING (true);
...
CREATE POLICY "maza_learning_proposals_update"
  ON public.maza_learning_proposals
  FOR UPDATE
  TO authenticated
  USING (true)
...
-- ── 4. Comentários ────────────────────────────────────────────────────────
COMMENT ON TABLE public.maza_learning_proposals IS
  'Propostas de melhoria geradas pelo Learning Machine por módulo da Maza.';
COMMENT ON COLUMN public.maza_learning_proposals.modulo IS
  'Módulo de origem: pessoas | operacao | financeiro';
COMMENT ON COLUMN public.maza_learning_proposals.tipo IS
  'Tipo de proposta: faq | prompt | processo | integracao';
COMMENT ON COLUMN public.maza_learning_proposals.prioridade IS
  'Prioridade: alta | media | baixa';
COMMENT ON COLUMN public.maza_learning_proposals.evidencia IS
  'Dado ou métrica que sustenta a proposta (ex: "23% de turnover no mês")';
COMMENT ON COLUMN public.maza_learning_proposals.impacto_estimado IS
  'Efeito esperado se a proposta for implementada';
COMMENT ON COLUMN public.maza_learning_proposals.status IS
  'pending = aguardando decisão | approved = aprovado | dismissed = descartado';
COMMENT ON COLUMN public.maza_learning_proposals.executed_at IS
