# AUDITORIA SHELL · Maza — Junho/2026

> Fase 1+2 do superprompt de auditoria. Somente leitura — nenhuma alteração de código foi feita.
> Escopo: repositório `maza` (monorepo: `apps/maza` + `packages/{auth,db,ui}` + `supabase/migrations`).
...
| Assumido | Realidade |
|---|---|
| `src/app` na raiz | Monorepo — app em `apps/maza/src/app`, pacotes compartilhados em `packages/` |
| CLAUDE.md com módulos documentados | **Não existe CLAUDE.md.** Só `AGENTS.md` genérico (aviso de versão do Next) e `apps/maza/README.md` = template padrão do Next, zero conteúdo do projeto |
| ~53 rotas no shell | Shell tem **28 páginas locais** + **7 zonas externas** (multi-zone via rewrites em `apps/maza/next.config.ts:1-9`): financeiro, pessoas, operacao, compras, comercial (ruptura), marca, inteligencia. O nav (`src/lib/nav-config.ts`, 82 hrefs) aponta 11 rotas locais e ~71 servidas pelas zonas |
| Link Trato 404 | **"Trato" não existe neste repo** (grep completo). Se o 404 é real, está numa zona externa — fora deste codebase |
| Cores de marca placeholder | Não encontrado placeholder no shell — design tokens MAZA completos em `globals.css` (Carvão/Creme/Brasa/Pedra/Ouro). Se há placeholder, é em zona externa |
...
| **UI / UX** | **7/10** | Design system coeso (tokens CSS, Fraunces+Instrument, empty states contextuais) — contraste de `--text-3` no limite do AA, botões inline vs `@maza/ui` inconsistentes, tabelas sem UX mobile |
...
1. **Middleware com auth desativado** — `apps/maza/middleware.ts:5-8`
   Comentário literal: `// AUTH DESATIVADO — acesso livre sem login`. Todo o painel estava público no domínio anterior. *Esforço: médio (depende de resolver o bug de auth original).*
...
4. **Usuários de bypass seedados e hardcoded** — `supabase/migrations/039_seed_bypass_user.sql:24`, `apps/maza/src/lib/pessoas/ponto-actions.ts:65-66`, `apps/maza/src/lib/pessoas/actions.ts:1268-1269`
...
5. **`/api/auth-debug` público expõe cookies de sessão** — `apps/maza/src/app/api/auth-debug/route.ts`
...
6. **`/api/intelligence/insight` sem auth consumindo Anthropic** — `apps/maza/src/app/api/intelligence/insight/route.ts:24`
   POST público que chama Claude com `ANTHROPIC_API_KEY` — abuso de créditos/DoS financeiro. *Esforço: trivial.*

7. **Webhook do orquestrador sem verificação de assinatura** — `apps/maza/src/app/api/orchestrator/webhook/route.ts:11`
...
8. **RLS `USING (true)` em tabelas sensíveis** — `043_employee_auth.sql:24` (hashes de senha mobile!), `045_theo_tickets.sql:25`, `047_candidates.sql:30`, `048_candidatos_maya.sql:29`, `073_maza_learning_proposals.sql:37,47`; view `tips_records` sem `security_invoker` em `044_mobile_views.sql`
   Corrigir via **migration aditiva nova** (DROP POLICY IF EXISTS + CREATE com `maza_has_role_for_brand`/`maza_can_write_*`), nunca ALTER. *Esforço: médio.*

### P1 — Qualidade estrutural

9. **Build ignora erros de TypeScript e ESLint** — `apps/maza/next.config.ts:13-14`
...
2. **Padronização de botões** — eliminar botões com `style={{}}` inline (ex.: `EmptyState.tsx:78-103`, `eventos/page.tsx:87-92`) em favor de `@maza/ui/button`; tokenizar os hex soltos (`#22C55E` em `Sidebar.tsx:278`, `#0a0a0a`, `#EF4444` → `--color-success/--color-danger`).
...
4. **Notificações cross-zone** — o bell só vê o schema local; definir contrato (tabela `maza_alerts` já tem TODO em `lib/discord/alert-notifier.ts:3`) para zonas publicarem alertas no mesmo feed.
...
**Objetivo: a aplicação Maza exige login real e RLS volta a valer.**
...
4. Migration aditiva `074_security_hardening.sql`: DROP POLICY IF EXISTS + policies reais para employee_auth, candidates, candidatos_maya, theo_tickets, maza_learning_proposals; `CREATE OR REPLACE VIEW tips_records ... WITH (security_invoker = true)`; desativar conta técnica de bypass (aditivo: revogar, não deletar).
