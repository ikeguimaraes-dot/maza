# Integração dos Portais HOS na Maza

Mapeamento das 3 plataformas externas e plano de absorção dentro da Maza.
...
Site institucional do portfólio do Grupo Hospitalidade São Paulo. Apresenta as marcas com tagline, ticket médio e conceito. **Marketing externo**, não operacional. Não pertence à Maza.

---

## 2. Estado atual — o que já existe na Maza
...
- **Multi-tenant**: `groups` → `brands` → `units` (já existe: holding MAZA → marca Madonna Cucina → unidade SP Itaim no seed)
- **RBAC**: `roles` (`founder`, `cfo`, `gm`, `pessoas`, `chef`, `comprador`, `colaborador`, `socio_readonly`) + `user_roles` escopados a unit/brand/group; helpers `maza_is_founder()`, `maza_has_role_for_unit()`, `maza_has_role_for_brand()`
...
- **ETL**: [scripts/etl-hos-to-maza.ts](../scripts/etl-hos-to-maza.ts) já lê o **HOS RH** (Supabase `afxsrcezmetipzgosdvb`) e migra employees/payslips/absences/warnings/dependents para o MAZA
...
A regra: **Maza = sistema operacional central**. Os sites externos viram módulos internos ou são descontinuados quando redundantes.
...
**Resultado**: os sites Netlify viram um módulo na Maza.

### Fase E2 — Módulo Eventos / O.S. (5–8 dias)
Reimplementar o módulo de eventos dentro da Maza reutilizando RBAC e multi-tenant.
...
- Migration `008_eventos.sql`:
  ...
  - RLS: `maza_has_role_for_unit(unit_id)` em todas — espelha padrão pessoas
...
**Resultado**: `hos-oseventos.vercel.app` desativado, dados consolidados no MAZA.
...
O portal institucional é um site público de marketing. **Não migrar para a Maza.** Pode ficar separado ou ser portado para um Next.js em `apps/marketing/` quando houver demanda.
