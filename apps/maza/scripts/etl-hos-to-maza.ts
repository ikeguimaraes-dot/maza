...
// - Write no MAZA autenticado como founder via password (RLS aceita)
...
const MAZA_URL = process.env.NEXT_PUBLIC_SUPABASE_URL;
const MAZA_SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY;

if (!MAZA_URL || !MAZA_SERVICE_KEY) {
  console.error(
...
const maza = createClient(MAZA_URL, MAZA_SERVICE_KEY, {
...
async function main() {
  console.log("═══════════════════════════════════════════════");
  console.log("ETL HOS RH → Maza");
  console.log("═══════════════════════════════════════════════\n");

  console.log(`✓ MAZA cliente service_role (RLS bypass)`);
  ...
  // 3a) Group MAZA
  const { data: mazaGroup, error: gErr } = await maza
    .from("groups")
    .select("id, name")
    .eq("slug", "maza")
    .maybeSingle();
  if (gErr) throw new Error(`Falha resolvendo group MAZA: ${gErr.message}`);
  if (!mazaGroup) throw new Error("Grupo configurado não existe na Maza.");
  const mazaGroupId = (mazaGroup as { id: string }).id;

  // 3b) Brand "Meet & Eat"
  let brandId: string;
  const { data: existingBrand } = await maza
    .from("brands")
    .select("id, name")
  ...
  if (existingBrand) {
    ...
    const { data: newBrand, error: bErr } = await maza
      .from("brands")
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .insert({
        group_id: mazaGroupId,
  ...
  let unitId: string;
  const { data: existingUnit } = await maza
    .from("units")
  ...
  if (existingUnit) {
    ...
    const { data: newUnit, error: uErr } = await maza
      .from("units")
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      .insert({
        brand_id: brandId,
  ...
  // Mapas: HOS id → MAZA id (chave principal pra FKs depois)
  //        cpf normalizado → MAZA id (debug)
  const hosIdToMazaId = new Map<string, string>();
  const cpfToMazaId = new Map<string, string>();
  ...
  for (let i = 0; i < employees.length; i++) {
    ...
    try {
      ...
      const { data: existing } = await maza
        .from("employees")
        .select("id")
        .eq("cpf", cpf)
        .maybeSingle();
      ...
      if (existing && (existing as { id: string }).id) {
        ...
        const { error: updErr } = await maza
          .from("employees")
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          .update(payload as any)
          .eq("id", id);
        if (updErr) throw updErr;
        hosIdToMazaId.set(e.id, id);
        cpfToMazaId.set(cpf, id);
        counters.employees_updated++;
        console.log(`  ↻ ${i + 1}/${employees.length} ${e.full_name} — atualizado`);
      } else {
        const { data: inserted, error: insErr } = await maza
          .from("employees")
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  for (const d of hosDeps) {
    const mazaEmpId = hosIdToMazaId.get(d.employee_id);
    if (!mazaEmpId) {
    ...
    try {
      ...
      const { data: existing } = await maza
        .from("dependents")
        .select("id")
        .eq("employee_id", mazaEmpId)
        .eq("nome", d.name)
        .maybeSingle();
      if (existing) continue;

      const { error } = await maza.from("dependents").insert({
        employee_id: mazaEmpId,
        nome: d.name,
  ...
  for (const [hosEmpId, totalMin] of minutesByHosEmp) {
    const mazaEmpId = hosIdToMazaId.get(hosEmpId);
    if (!mazaEmpId) continue;
    try {
      const ultimo = lastDateByHosEmp.get(hosEmpId) ?? null;
      const { error } = await maza.from("time_bank_balance").upsert(
        {
          employee_id: mazaEmpId,
  ...
  for (const p of hosPs) {
    const mazaEmpId = hosIdToMazaId.get(p.employee_id);
    if (!mazaEmpId) {
    ...
    try {
      ...
      const { error } = await maza.from("payslips").upsert(
        {
          employee_id: mazaEmpId,
          competencia,
  ...
  for (const a of hosAbs) {
    const mazaEmpId = hosIdToMazaId.get(a.employee_id);
    if (!mazaEmpId) continue;
    try {
      ...
      const { error } = await maza.from("absences").insert({
        employee_id: mazaEmpId,
        data: a.date,
        tipo: a.type,
  ...
  for (const w of hosWarn) {
    const mazaEmpId = hosIdToMazaId.get(w.employee_id);
    if (!mazaEmpId) continue;
    try {
      const { data: existing } = await maza
        .from("warnings")
      ...
      const { error } = await maza.from("warnings").insert({
        employee_id: mazaEmpId,
        nivel: w.level,
  ...
  const out = {
    ...
    counters,
    errors,
    hosIdToMazaId: Object.fromEntries(hosIdToMazaId),
    cpfToMazaId: Object.fromEntries(cpfToMazaId),
  };
