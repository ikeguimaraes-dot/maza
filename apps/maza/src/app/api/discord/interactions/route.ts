...
import { executeMazaAgent } from '@/lib/orquestrador/maza-agents'
...
export async function POST(req: NextRequest) {
  ...
  if (body.type === APPLICATION_COMMAND) {
    ...
    // MAZA AI Agent commands — all deferred (async via after())
    const MAZA_AGENT_COMMANDS: Record<string, string> = {
      financeiro: 'dados',
      cardapio: 'itens',
      copy: 'texto',
      conteudo: 'marca',
    ...
    if (commandName in MAZA_AGENT_COMMANDS) {
      const inputOption = MAZA_AGENT_COMMANDS[commandName]!
      const input: string = body.data?.options?.find((o: any) => o.name === inputOption)?.value ?? ''
      const token: string = body.token
      after(async () => {
        await executeMazaAgent(commandName, input, token)
      })
      return Response.json({ type: 5 }) // DEFERRED_CHANNEL_MESSAGE_WITH_SOURCE
    }
    ...
    if (decision === 'approve' && process.env.MAZA_API_SECRET) {
      ...
      fetch(`${baseUrl}/api/orquestrador/approve`, {
        ...
        body: JSON.stringify({
          job_id: runId,
          secret: process.env.MAZA_API_SECRET,
        }),
      }).then(async (res) => {
        if (res.ok) {
