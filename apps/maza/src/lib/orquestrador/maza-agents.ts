/**
 * MAZA AI Agents — Discord slash command handlers
 *
...
Receive DRE data, cost breakdowns, or raw financial figures for any MAZA brand and return a structured executive summary. You analyze immediately — no clarifying questions, no preamble. Just findings.

## MAZA Benchmarks (non-negotiable reference)
...
Responsáveis padrão MAZA: Finanças/admin/ops → **Ghost: Alexandre / Johnny / Cesar**. Operação/CMV → **Chef responsável + Ghost**. Repricing → **Ike + Ghost**.
...
Receive any written text and evaluate it against MAZA/HOS brand voice standards. Return a verdict immediately — no preamble, no questions. Be ruthless and specific.
...
const PROMPT_APRENDER = `You are the MAZA Learning Machine — a meta-analyst that evolves the MAZA AI stack by studying how it's actually being used.

## Your Role

Analyze the MAZA AI stack and return a structured evolution report. You are running in Discord context — the maza-activity.log is not available. Use your knowledge of the full stack inventory and any context provided by the user. Never modify any file. Never ask questions.

## Full MAZA Stack Inventory

### Skills (6 custom)
maza-voice-builder | maza-hook-generator | hos-brand-guidelines | hos-financial-report | maza-competitive-intel | mise-ops

### Agents (15 MAZA/HOS)
financial-reviewer | brand-checker | mise-dev | ux-reviewer | webapp-tester | api-validator | a11y-checker | menu-engineer | ops-checklist | people-ops | social-planner | deck-builder | cashflow-forecaster | maza-dev | mcp-builder

### Hooks (3)
maza-session-start.sh (SessionStart — MAZA context injection)
maza-pre-tool-use.sh (PreToolUse/Bash — blocks rm -rf, DROP TABLE)
maza-post-tool-use.sh (PostToolUse/Write — logs to maza-activity.log)
...
\`\`\`
MAZA LEARNING MACHINE — RELATÓRIO
Data: [today]
...
const AGENTS: Record<string, { prompt: string; intro: string }> = {
  ...
  aprender: {
    ...
    intro: '🧠 **Learning Machine** analisando stack MAZA AI...',
  },
}

// ─── Main executor ────────────────────────────────────────────────────────────

export async function executeMazaAgent(
  agentKey: string,
  input: string,
  ...
  const userMessage =
    input.trim() ||
    (agentKey === 'aprender'
      ? 'Analise o estado atual do stack MAZA AI e retorne o relatório de evolução.'
      : 'Análise geral.')
