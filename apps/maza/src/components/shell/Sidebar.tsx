...
export function Sidebar() {
  ...
  useEffect(() => {
    const syncSessionCookie = () => {
      ...
      if (auth) {
        window.localStorage.setItem("maza_auth_browser_backup", auth);
        return;
      }
      const backup = window.localStorage.getItem("maza_auth_browser_backup");
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("maza:toggleSidebar", onToggle);
  }, []);
